 ...
 *   2. The readonly lock: browsers and password managers skip readonly fields
 *      for both autofill AND the saved-credential suggestion dropdown. Each
 *      field stays readonly through the focus event and unlocks only once the
 *      user actually clicks through, types, or taps (touch devices).
 *
...
(function () {
  ...
  function lockUntilUserInput(el) {
    ...
    // The field must stay readonly THROUGH the focus event: browsers decide
    // whether to show the saved-username/password suggestion dropdown exactly
    // when a field gains focus, and they skip readonly fields. So we unlock
    // only after that moment.
    //
    // Mouse/desktop: unlock on click, which fires AFTER focus.
    el.addEventListener('click', unlock, { once: true });

    // Keyboard/typing: unlock on the first keystroke or text insertion.
    el.addEventListener('keydown', unlock, { once: true });
    el.addEventListener('beforeinput', unlock, { once: true });

    // Touch devices: a readonly field won't raise the on-screen keyboard, so
    // unlock before the browser focuses it.
    el.addEventListener('touchstart', unlock, { once: true, passive: true });
    el.addEventListener('pointerdown', function (e) {
      if (e.pointerType === 'touch' || e.pointerType === 'pen') unlock();
    }, { once: true });
  }
